...
| **PR3b** ✅ | Consumer domains (media_player + switch) | `media_player.py` + `switch.py` on a shared consumer base extracted from `remote.py`; honest off-semantics; cross-domain transition. Validated on real HA via the PR3d smoke (media_player features/source_list/select_source work). | Off-semantics; optimistic+Restore; cross-domain move; existing tests green. |
...
| **PR3d** ✅ | Canonical transmitter identity + reconcile fixes | Identity desync + dropped-reconcile race fixed. **Confirmed on real HA 2026-06-17:** single canonical transmitter (orphan pruned), Sony auto-migrated to canonical key, real `volume_up`/`select_source`/`remote.send_command` work, feature-burst keeps full `source_list` (trailing-edge), ref resolve/validate/clear work. | ref resolver; reconcile orphans; write-path validation; no masking fallback; v3→v4 migration; trailing-edge reconcile; transmitters in `list_commands`. |
| **PR-MT** ✅ | Multi-transmitter (hub + subentries) | Single hub config entry; transmitters as config subentries; owner-election deleted; both lifecycle edges closed. Design + migration in [PLAN-hub-subentries-migration.md](PLAN-hub-subentries-migration.md). **Confirmed on real HA 2026-06-17:** N=1 migration reshaped the legacy entry into hub+subentry, consumer entities survived without dupes, real `volume_up`/`remote.send_command` produced physical device reaction. Post-smoke fix: `media_player.update_spec` reordered so a single `update_command` refreshes `source_list`/features immediately (no state-write lag). | Config/subentry flow; emitters per subentry; consumer on hub; multi-entry→hub migration; update_spec refresh; existing tests green (82). |
